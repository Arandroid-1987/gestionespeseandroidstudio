...
public class BilanciFragment extends Fragment implements OnClickListener,
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		exportButton = (FloatingActionButton) rootView.findViewById(R.id.fab);
	...
	protected void setCustomDatesAndLabel() {
		startDate = DateUtils.getDateString(fromYear, fromMonth + 1, fromDay);
		endDate = DateUtils.getDateString(toYear, toMonth + 1, toDay);
			...
			if (graficoSelezionato.equals(grafici[i])) {
				...
				case 1:
					if (periodoSelezionato != null
							&& (periodoSelezionato.equals(periodi[0])
									|| periodoSelezionato.equals(periodi[1]) || periodoSelezionato
										.equals(periodi[2]))) {
						Toast.makeText(
								context,
								getResources().getString(
						...
						String[] labels = new String[ricaviSeries
								.getItemCount()];
						for (int j = 0; j < labels.length; j++) {
							labels[i] = getString(R.string.periodo) + " "
									+ (i + 1);
						}
	...
	private boolean exportPDF(File file) {
		String path = Environment.getExternalStorageDirectory()
				.getAbsolutePath() + "/gestionespese";
	...
	private void createAndShowExportDialog() {
		...
		exportPDF = (Checkable) v.findViewById(R.id.exportPDF);
		exportCSV = (Checkable) v.findViewById(R.id.exportCSV);
		exportExcel = (Checkable) v.findViewById(R.id.exportExcel);
		exportText = (Checkable) v.findViewById(R.id.exportText);
		separatorCSV = (Spinner) v.findViewById(R.id.separatorCSV);
		separatorCSVLayout = (LinearLayout) v
		...
		String[] separators = new String[] { CSVExport.DEFAULT, CSVExport.EXCEL };

		for (String name : separators) {
