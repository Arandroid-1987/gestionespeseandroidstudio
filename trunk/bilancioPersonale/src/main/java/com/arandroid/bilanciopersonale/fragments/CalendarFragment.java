...
import com.arandroid.bilanciopersonale.services.NotificationService;
		...
		calendarView
				.setOnDayClickListener(new ExtendedCalendarView.OnDayClickListener() {
					...
					public void onDayClicked(AdapterView<?> adapter, View view,
							int position, long id, Day day, ArrayList<Day> allDays) {
//						ArrayList<VoceBilancio> events = day.getEvents();
						Intent intent = new Intent(context,
								RiepilogoGiornalieroActivityNew.class);
//						intent.putExtra("lista", events);
						intent.putExtra("giorni", allDays);
						intent.putExtra("position", position);
						intent.putExtra("baseDate",
								DateUtils.getDateString(day.getYear(),
										day.getMonth() + 1, day.getDay()));
						context.startActivity(intent);
					}
				});
		checkVociProgrammateScadute();
		data = DateUtils.getDateString(new Date());
	...
	public void onResume() {
		super.onResume();
		calendarView.refreshCalendar();
		int minutes = 30; // deve essere 30; 1 per scopi di test
		AlarmManager am = (AlarmManager) context
				.getSystemService(Activity.ALARM_SERVICE);
		Intent i = new Intent(context, NotificationService.class);
		PendingIntent pi = PendingIntent.getService(context, 0, i, 0);
		am.cancel(pi); // by my own convention, minutes <= 0 means notifications
						// are disabled
		if (minutes > 0) {
			am.setInexactRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP,
					...
					minutes * 60 * 1000, pi);
			// am.setInexactRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP,
			// SystemClock.elapsedRealtime() + minutes * 1 * 1000,
			// minutes * 1 * 1000, pi);
		}
	}
